...
//	def ++[B >: A](oth: List[B]): List[A]
//	def reverse: List[A]
//	def updated[B <: A](n: Int, x: B): List[A]
...
//	def take(n: Int) = Nil
//	def drop(n: Int) = Nil
//	def ++[B >: A](oth: List[B]) = oth
//	def reverse = Nil
//	def updated[B](n: Int, x: B) = throw new Exception("Cannot updated an empty list")
//	def indexOf[B](x: B) = throw new Exception("Index out of bound, empty list")
//	def contains[B](x: B) = false
...
//	}
//
//	def ++(oth: List[A]): List[A] = {
//		def helper(xs: List[A], ys: List[A]): List[A] = {
//			xs match {
//				case Nil => ys
//				case Cons(head, tail) => Cons(head, helper(tail, ys))
//			}
//		}
//		helper(this, oth)
//	}
...
//		case Cons(head, tail) => tail.reverse ++ Cons(head, Nil)
//	}
//	def updated(n: Int, x: A): List[A] = {
//		def helper(n: Int, xs: List[A]): List[A] = xs match {
//			case Cons(h, tail) if n == 0 => Cons(x, tail)
//			case Cons(h, tail) => Cons(h, helper(n - 1, tail))
//			case _ => throw new Exception("Index out of bound exception")
//		}
//		helper(n, this)
//	}
//	def indexOf(x: A): Int = {
//		def helper(i: Int, xs: List[A]): Int = xs match {
//			case Cons(h, t) if (h == x) => i
//			case Cons(h, t) => helper(i + 1, t)
//			case _ => throw new Exception("No such element")
//		}
//		helper(0, this)
//	}
//	def contains(x: A): Boolean = this match {
//		case Cons(h, t) if (h == x) => true
//		case Cons(h, t) => t.contains(x)
//		case _ => false
//	}
