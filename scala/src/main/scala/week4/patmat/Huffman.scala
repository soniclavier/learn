...
object Huffman {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(encode(tree)(decode(tree, bits)))
    println(convert(tree))
    println(quickEncode(tree)(List('a','d','b')))
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def iter(subTree: CodeTree, text: List[Char]): List[Bit] = {
      if (text.isEmpty) List()
      else {
        subTree match {
          case Leaf(c, w)       => if (text.tail.isEmpty) List() else iter(tree, text.tail)
          case Fork(l, r, c, w) => if (chars(l).contains(text.head)) 0 :: iter(l, text) else 1 :: iter(r, text)
  ...
  def convert(tree: CodeTree): CodeTable = {
    ...
    def iter(acc: List[Bit], subTree: CodeTree): CodeTable = {
      subTree match {
        case Leaf(c, w)       => List((c, acc))
        case Fork(l, r, c, w) => mergeCodeTables(iter(acc ++ List(0),l), iter(acc ++ List(1),r))
      }
    }
    iter(List(),tree)
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def iter(table: CodeTable, text: List[Char]): List[Bit]= {
      if (text.isEmpty) List()
        ...
        val head = text.head
        table.filter(_._1 == head).head._2.union(iter(table,text.tail))
      }
    }
    iter(table,text)
  }
